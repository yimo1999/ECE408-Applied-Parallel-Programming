...
__global__ void fusion_kernel(const float *input, float *output, const float *mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K){
    ...
    for(int q = 0; q < ceil(1.0 * Channel * K * K / TILE_WIDTH); q++){
      ...
      i0 = r % (K * K) % K + col % Width_out;
      i1 = r % (K * K) / K + col / Width_out;
      i2 = r / (K * K);
      // Batch
      i3 = bz;
      ...
      if(col < (Height - K + 1) * (Width - K + 1) && (q * TILE_WIDTH + ty) < Channel * K * K){
        subTileN[ty][tx] = in_4d(i3, i2, i1, i0);
      }else{
        subTileN[ty][tx] = 0.0f;
      }
    ...
    if(row < Map_out && col < (Height - K + 1) * (Width - K + 1)){
      out_4d(bz, row, col / Width_out, col % Width_out) = Pvalue;
    }
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    dim3 dimGridFusion(ceil(1.0 * Width_out * Height_out / TILE_WIDTH), ceil(1.0 * Map_out / TILE_WIDTH), Batch);
    dim3 dimBlockFusion(TILE_WIDTH, TILE_WIDTH, 1);

    // std::cout << "///////////////////////Map_out: " << Map_out << "/////////////////////////" << std::endl;
    if(Map_out != 16){
      conv_forward_kernel_shared<<<dimGrid, dimBlock>>>(device_output, device_input, device_mask, Batch, Map_out, Channel, Height, Width, K);
    }else{
      fusion_kernel<<<dimGridFusion, dimBlockFusion>>>(device_input, device_output, device_mask, Batch, Map_out, Channel, Height, Width, K);
    }
