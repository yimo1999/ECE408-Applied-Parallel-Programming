...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    int by = blockIdx.y;
    int bz = blockIdx.z;
    int h = by / w_size * TILE_WIDTH + threadIdx.y;
    int w = by % w_size * TILE_WIDTH + threadIdx.x;
    ...
    if(h < Height_out && w < Width_out){
      ...
      for(int c = 0; c < Channel; c++){
        for(int p = 0; p < K; p++){
          for(int q = 0; q < K; q++){
            // acc += in_4d(bz, c, h + p, w + q) * mask_4d(bx, c, p, q);
            acc = __hadd2(__hmul2(__float2half2_rn(in_4d(bz, c, h + p, w + q)), __float2half2_rn(mask_4d(bx, c, p, q))), acc);
          }
        }
      }
      out_4d(bz, bx, h, w) = __low2float(acc);
    }
