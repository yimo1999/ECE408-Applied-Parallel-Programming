...
__global__ void total(float *input, float *output, int len) {
  ...
  int firstBlockidx = start + t;
  int secondBlockidx = BLOCK_SIZE + start + t;



  if(firstBlockidx < len){
      partialSum[t] = input[firstBlockidx];
  }else{
      partialSum[t] = 0.0f;
  }

  if(secondBlockidx < len){
      partialSum[BLOCK_SIZE + t] = input[secondBlockidx];
  }else{
